...
[[nodiscard]] double &Matrix4::at(u8 i) {
    if (i > 15) throw Exception(ExitCode::Value::OutOfBounds);
    return m_data[i];
}
[[nodiscard]] const double &Matrix4::at(u8 i) const {
    if (i > 15) throw Exception(ExitCode::Value::OutOfBounds);
    return m_data[i];
}
[[nodiscard]] double &Matrix4::at(u8 i, u8 j) {
    if (i > 3) throw Exception(ExitCode::Value::OutOfBounds);
    if (j > 3) throw Exception(ExitCode::Value::OutOfBounds);
    return m_data[i * 4 + j];
}
[[nodiscard]] const double &Matrix4::at(u8 i, u8 j) const {
    if (i > 3) throw Exception(ExitCode::Value::OutOfBounds);
    if (j > 3) throw Exception(ExitCode::Value::OutOfBounds);
    return m_data[i * 4 + j];
}
